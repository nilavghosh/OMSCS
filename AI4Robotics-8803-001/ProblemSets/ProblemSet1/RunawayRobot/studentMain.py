...
def estimate_next_pos(measurement, OTHER=None):
    ...
    """Structure of OTHER
    [[previous measurements], dist_moved, angle_moved, ctr]"""
    xy_estimate = (0,0)
    ctr = -1
    if OTHER is None:
       ...
       xy_estimate = measurement
        ...
        ctr = OTHER[3]
        if(ctr == 0):
            dist = distance_between(OTHER[0][0], measurement)
        ...
        if(ctr == 1):
            ...
            angle = angle2 - angle1 
            OTHER[2] = angle
            x = m2x + OTHER[1] * cos(angle*(ctr+2))
            y = m2y + OTHER[1] * sin(angle*(ctr+2))
            xy_estimate = (x,y)
        if(ctr > 1):
            angle = OTHER[2]
            mx,my = measurement
            x = mx + OTHER[1] * cos(angle*(ctr+2))
            y = my + OTHER[1] * sin(angle*(ctr+2))
            xy_estimate = (x,y)
    ctr += 1
    OTHER[3] = ctr
...
# This is a demo for what a strategy could look like.  This one isn't very
# good.
...
test_target = robot(2.1, 4.3, 0.0, 2 * pi / 98.0, 1.5)
...
if __name__ == "__main__":
    demo_grading2(estimate_next_pos,test_target,None)