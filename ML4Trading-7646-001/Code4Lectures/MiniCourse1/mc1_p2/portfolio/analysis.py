...
def get_portfolio_value(prices, allocs, start_val=1):
    ...
    """
    pos_val = prices/prices.ix[0,:]
    pos_val = pos_val.mul(allocs, axis=1)*start_val
...
def get_portfolio_stats(port_val, daily_rf=0, samples_per_year=252):
    ...
    daily_returns[1:] = (daily_returns[1:]/ daily_returns[:-1].values) - 1.0
    daily_returns.ix[0] = 0.0
    daily_returns = daily_returns[1:]
    avg_daily_ret =  daily_returns.mean()
    std_daily_ret = daily_returns.std()
    sharpe_ratio = (252.0**.5) * (daily_returns-daily_rf).mean()/std_daily_ret
    # TODO: Your code here
...
def plot_normalized_data(df, title="Normalized prices", xlabel="Date", ylabel="Normalized price"):
    ...
    plot_data(df_normalized,"Stock prices","Date","Price")
    #TODO: Your code here
